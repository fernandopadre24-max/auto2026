...
const formSchema = z.object({
  partName: z
    .string()
    .min(2, { message: 'O nome da peça deve ter pelo menos 2 caracteres.' }),
  ...
  technicalSpecifications: z
    .string()
    .min(10, { message: 'As especificações devem ter pelo menos 10 caracteres.' }),
  ...
  useEffect(() => {
    if (part) {
      form.reset({
        ...
        inventoryLevel: String(part.stock),
  ...
  async function onSubmit(values: PartFormData) {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: 'Erro',
        description: `Erro ao atualizar peça: ${errorMessage}`,
      });
